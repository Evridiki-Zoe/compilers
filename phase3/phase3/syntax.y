...
//-------------------------Types of some rules----------------------------------
%type<intValue>         INTEGER if_start whilestart whilecond funcstart for_cond
%type<intValue>  		for_end for_elist epsilon

%type<floatValue>       FLOAT

%type<stringValue>      IDENTIFIER STRING

%type<expression>      	arexpr boolResexpr funcname funcdef call expr const term
%type<expression>		primary number assignmexpr lvalue elist objectdef
%type<expression>		multi_exprs_for_table elist_for_table TRUE FALSE NIL
%type<expression>		indexed indexedelem  multi_indexedelem member multi_exprs
...
multi_stmts : 				{current_rvals = 0; }
				stmt 		{rvalues = current_rvals;}
				multi_stmts { printf(RED "stmt multi_stmt\n" RESET); }
...
stmt	:
 			expr SEMICOLON  {
					...
					/*Insert extra quads if we want to calculate the expr*/
					if (exprflag) {
      ...
      | 	returnstmt  {
		  ...
		  if( insideFunc > 0) push_R((int)QuadNo-1); //push to return stack for backpatch
		  else {
      ...
      |		BREAK SEMICOLON {
	                ...
	                if( insideLoop > 0) {
							...
							push_B((int)QuadNo-1); //push to break stack for backpatch
	               	} else {
      ...
      |		CONTINUE SEMICOLON {
                    ...
                    if( insideLoop > 0) {
                    		emit(jump,NULL,NULL,NULL,yylineno,999);
							push_C((int)QuadNo-1); //push to continue stack for backpatch
                	} else {
...
expr :
			assignmexpr {
					 ...
					 if($1->type == tableitem_e){
						 ...
						 $$ = new_expr(var_e,newnode,NULL,0,"",'\0',NULL);
	...
	|  		expr AND  	{
					...
					/*Create the quads to evaluate the expr if not evaluated*/
					if (!$1->apotimimeno) {
						struct expr* true_expr = new_expr(constbool_e,true_expr_sym,NULL,0,"",1,NULL );

						emit(if_eq,$1,true_expr,NULL,yylineno,999);
						emit(jump,NULL,NULL,NULL,yylineno,999);
			...
			epsilon expr	{
					...
					if (!$5->apotimimeno) {
					  	...
					  	emit(if_eq,$5,true_expr,NULL,yylineno,999);
					  	emit(jump,NULL,NULL,NULL,yylineno,999);
					...
					//Create new expr for the result
		  			$$ = smallFunc(boolexp_e);

		  			//Backpatch the truelist for expr1
	...
	|		expr OR		{
			...
			epsilon expr	{
					...
					//Backpatch the falselist for expr1
...
/*Empty rule to store the current quadno*/
...
boolResexpr :
		...
		|  expr NEQUAL expr {
					...
					emit(if_not_eq,$1,$3,NULL,yylineno,999);
					emit(jump,NULL,NULL,NULL,yylineno,999);
...
assignmexpr   :
			...
			lvalue {
					...
					if(!arrayFlag && ref)   check_for_funcname($1->sym->value.var->name);
			...
			EQ expr {
					...
					if($1->type == tableitem_e){
						...
						//allagi
					    $$ = new_expr(tableitem_e,$1->index->sym,$1,0,"",'\0',NULL);
...
lvalue   :
			IDENTIFIER {
				 		printf( "lvalue:: id %s\n", $1);
						//allagi
						$$=new_expr(var_e,insertVar($1, yylineno, scope),NULL,0,"",'\0',NULL);
					}
         | LOCAL IDENTIFIER {
						printf(RED "lvalue:: local identifier\n" RESET);
						//allagi
						$$=new_expr(var_e,localVar($2, yylineno, scope),NULL,0,"",'\0',NULL);
					}
         | DCOLON IDENTIFIER {
		            	//allagi
						printf( RED "lvalue:: doublecolon\n" RESET);
				        $$=new_expr(var_e,global_exists($2),NULL,0,"",'\0',NULL);
...
member :
         ...
         | call DOT IDENTIFIER {
						//allagi Den ksero an xreiazetai
						...
						$1->index = new_expr(tableitem_e,tmp_symbol_id,NULL,0,"",'\0',NULL);
...
call   :
			call L_PARENTHES elist R_PARENTHES {
...
callsuffix :
				normcall 	{ printf(RED"callsuffix:: (elist)\n"RESET); $$ = $1;}
...
methodcall : DOTS IDENTIFIER L_PARENTHES elist R_PARENTHES {
...
elist : expr multi_exprs {
						//allagi , den nomizo na mpainoun ta next or smthing
	 					args++;
      ...
      | /*emty*/ {
						...
						args = 0;
						$$ = new_expr(var_e,NULL,NULL,0,"",'\0',NULL);
      }
      ;

multi_exprs	:  COMMA expr multi_exprs {
						args++;
						printf("multiexpr commma expr(%s) exprs\n", $2->sym->value.var->name);\
						...
						$$ = new_expr(var_e,newnode,NULL,0,"",'\0',$3);

					}
      |  /*empty*/ {
						printf(RED "multi exprsessions: empty\n" RESET);
						$$= new_expr(var_e,NULL,NULL,0,"",'\0',NULL); //to teleutaio eina null
      }
      ;
...
objectdef   :  L_SBRACKET elist_for_table R_SBRACKET  {
						...
						struct expr* tmp_table = smallFunc(newtable_e);
						...
						while(tmp->sym!= NULL) {
						    ...
						    struct expr* temp_elem = new_expr(var_e, insertVar(result ,  yylineno , scope),NULL,0,"",'\0',NULL);
            ...
            |  L_SBRACKET {
						emit(tablecreate,NULL,NULL,smallFunc(newtable_e),yylineno,0);
                }
            indexed R_SBRACKET {
	                   ...
	                   struct expr* tmp = $3;
	                    ...
	                    $$ = smallFunc(newtable_e);// epistrefw pros ta panw olo to table
            }
            ;


elist_for_table   :
			expr multi_exprs_for_table  {
						args++;
						...
						$$ = new_expr(tableitem_e,newnode,NULL,0,"",'\0',$2);


        }
        | /*emty*/ {
	     				$$ = new_expr(tableitem_e,NULL,NULL,0,"",'\0',NULL); //to teleutaio eina null
						printf(RED "elist:: empty\n" RESET);
						args = 0;
        }
        ;

multi_exprs_for_table 	:

 			COMMA expr  multi_exprs_for_table  {
                         args++;
...
multi_indexedelem	:
			COMMA indexedelem multi_indexedelem {
						printf(RED "multi_indexedelem:: comma indelem multi\n" RESET);
						$$= new_expr(tableitem_e,$2->sym,$2->index,0,"",'\0',$3);
...
indexedelem	  : L_CBRACKET expr COLON expr R_CBRACKET { //printf( "indelem {expr(%s) : expr(%s)}\n",$2->sym->value.var->name,$4->sym->value.var->name);
						//allagi
						emit(table_setelem,$2,$4,smallFunc(tableitem_e),yylineno,0);
						...
						$$ = new_expr(tableitem_e,newnode1,expr_for_symbol2,0,"",'\0',NULL);
...
funcname : IDENTIFIER {
		 ...
		 | /*empty*/ {
			 			//allagi
						emit(funcstart,smallFunc(2),NULL,NULL,yylineno,0);
						$$=tmpexpr;
		 }
...
if_start: IF L_PARENTHES expr {
						...
						emit(if_eq,$3,true_expr,NULL,yylineno,QuadNo+3);
						emit(jump,NULL,NULL,NULL,yylineno,999);
